...
function toBlogCardArticle(post: BlogPost, locale: Locale): BlogCardArticle {
  return {
    id: post.id,
    title: post.title,
    slug: post.slug,
...
export default async function BlogDetailView({
  slug,
  locale,
}: BlogDetailViewProps) {
  const t = getDictionary(locale).blogDetail;
          ...
          <div className="relative z-10 mx-auto max-w-[980px] min-w-0">
            <div className="mb-6 flex flex-col items-start gap-4 md:mb-8">
              ...
              <div className="inline-flex items-center gap-1.5 rounded-full border border-white/20 bg-white/10 px-3 py-1 text-xs font-medium backdrop-blur">
                ...
                {post.category ?? t.categoryFallback}
              </div>
            </div>
        ...
        <section className="bg-slate-50 px-6 py-16">
          <div className="mx-auto max-w-[1400px]">
            <div className="section-header fade-in visible">
              <div className="section-label">{t.relatedArticles}</div>
              <h2 className="section-title">{t.readNext}</h2>
            ...
            {similarPosts.length > 0 ? (
              <div className="grid min-w-0 gap-6 md:grid-cols-2">
                {similarPosts.map((similarPost) => (
                  <BlogCard
                    key={similarPost.id}
                    article={toBlogCardArticle(similarPost, locale)}
                    locale={locale}
                  />
                ))}
              </div>
